...
“按键与动作”旁的“流程图”按钮会单独打开事务流程编辑器，现有布局树、组件拖拽和属性表单保持不变。流程图只负责当前页面的 `keys.*` 与 `on.*`：普通动作按顺序连接，`if` 显示“满足/不满足”分支，外部 shell `call` 显示“逐行/退出”回调。

左侧节点库提供 Shell、直接程序、Service、变量设置与提取、列表追加/移动/删除、布尔切换、退格、进度、条件、通知、刷新和导航节点。窄屏时从工具栏打开节点库。可以把节点拖到画布，也可以右键创建；新节点默认保持游离，不改动已有执行链，完成连线后才会写回。`if` 的“继续”和事件的“结束”由编辑器自动维护。

右侧使用结构化参数表单。值可以在固定值、变量引用、模板和 JSON 之间切换，条件可以递归组合全部满足、任一满足与取反，高级 JSON 作为完整配置入口。`call` 表单覆盖阻塞方式、命令参数、环境变量、工作目录、标准流和返回值目标，可以只提取完整结果、stdout、stderr、行列表、JSON 或退出码。变量节点对应运行时 `set`，目标路径不存在时由 runtime 创建。

左侧“翻译文件”区域直接管理 `i18n` 字典。可以创建并注册 YAML/JSON 语言文件、搜索或新增嵌套翻译键，并在右侧编辑文本或 JSON 值。外部语言写回对应文件，内联语言写回 manifest；移除语言映射不会删除磁盘文件。语言文件在 VS Code 中修改后，翻译列表会实时同步。

拖动后的节点坐标只保存在 Webview 中，不会污染 YAML。点击连线后可从检查器删除，也可按 `Delete` 或 `Backspace`。断点、重复出口或循环会保留为 Webview 草稿，动作和连线只有在整张图合法时才会写回。`call: tasks.save` 这种标量 service 调用只显示“后续”出口；需要 `onLine`、`onExit` 时，应使用带 `sh` 或 `command` 的对象形式 shell call。
